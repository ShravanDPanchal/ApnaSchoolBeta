...
The **GSD (Get Shit Done)** protocol is an elite autonomous execution system. It prioritizes action, decisive execution, automated self-verification, and high signal-to-noise ratio.
...
2. **Define Milestones**: Split the user objective into sequential, manageable milestones:
   - *Milestone 1*: Foundation / Data Models / Contracts.
   - *Milestone 2*: Core Logic / Service Implementation.
   - *Milestone 3*: Integration / API Endpoints / UI Wiring.
   - *Milestone 4*: Automated Test Suites & Edge-Case Coverage.
...
1. Run project builds and test suites:
   ...
   - `npm run build` or typecheck commands (`tsc --noEmit`).
2. If failures occur:
...
2. Present a concise, structured delivery report:
